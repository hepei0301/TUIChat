	...
	getters: {
		toAccount: (state) => {
			...
			switch (state.currentConversation.type) {
				case 'C2C':
					return state.currentConversation.conversationID.replace(
						'C2C',
						''
					)
				case 'GROUP':
		...
		totalUnreadCount: (state) => {
			const result = state.conversationList.reduce(
				(count, conversation) => {
					// 当前会话不计算总未读
					if (
			...
			titleNotify(result)
			if (result !== window.messageNum) {
				window.messageNum = result
				if (result > 0) {
					window.parent.postMessage('腾讯云消息', '*')
				}
			}
			return result
		},
