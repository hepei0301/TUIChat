<template>
  <div class="friend-content"
       v-if="showFriendContent || showApplicationContent"
       @click="closeHandler">
    <div class="friend-box"
         v-if="showFriendContent">
      ...
      <p class="content-box"><span class="content-title">账号</span><span class="content-text">{{friendProfile.userID}}</span></p>
      ...
      <div class="content-box"
           v-show="friendType==='friendList'">
        <span class="content-title"
              v-show="!showEdit"
              style="line-height: 35px">所在分组</span>
        <span class="content-title"
              v-show="showEdit"
              style="line-height: 35px">添加到分组</span>
        <div class="text-content">
          <span class="content-text"
                v-show="!showEdit">{{groupName}}</span>
          <i class="el-icon-edit edit-icon"
             v-show="!showEdit"
             @click.stop="showEditHandler"></i>
        </div>
        <el-select v-if="showEdit"
                   v-model="addGroupName"
                   placeholder="选择分组"
                   @change="addToFriendGroup">
          <el-option v-for="item in friendGroupList"
                     @blur="showEdit = false"
                     :key="item.name"
                     :label="item.name"
                     :value="item.name">
      ...
      <p class="content-box"
         v-show="friendType==='groupFriend'"
         style="height: 40px">
        <span class="content-title"
              style="line-height: 35px">所在分组</span>
        <span class="content-text"
              v-show="!showEdit"
              style="line-height: 35px">{{groupName}}</span>
      </p>
      <div class="sendBtn"
           @click.stop="checkoutConversation(friendProfile.userID)">发送消息</div>
      <div class="delete-text"
           v-show="friendType==='groupFriend'"
           @click.stop="removeFromFriendGroup(friendProfile.userID)">从该群组中移除</div>
      <div class="delete-text"
           v-show="friendType==='friendList'"
           @click.stop="removeFromFriendList(friendProfile.userID)">删除该好友</div>
    </div>
    <div class="friend-box"
         v-if="showApplicationContent">
      <div class="profile-container">
      ...
      <div class="content-box"
           v-if="showRemark"><span class="content-title">备注名</span>
        <el-input style="display: inline-block"
                  ref="showRemark"
                  autofocus
      ...
      <p class="content-box">
        ...
        <el-tooltip class="item"
                    effect="dark"
                    :content="applicationContent.wording"
      ...
      <div class="application-box"
           v-if="!showRemark">
        <p class="application-refuse"
           @click="acceptHandler">同意</p>
        <p class="application-delete"
           @click="refuseFriendApplication(applicationContent.userID)">拒绝</p>
      </div>
      <div class="application-box"
           v-else>
        <p class="application-refuse"
           @click="acceptFriendApplication(applicationContent.userID)">确认</p>
        <p class="application-delete"
           @click="cancelHandler()">取消</p>
      </div>
    </div>
  </div>
</template>
  ...
  computed: {
    ...
    getSource() {
      return function (source) {
        console.log(12345, source.substring(15))
        return source.substring(15) === 'Unknow' ? '暂无' : source.substring(15)
      }
    },
  },
  methods: {
