<template>
  <div class="call-container"
    ...
    <div class="call"
         v-show="dialling || calling">
      <div class="title"
           v-if="dialling && currentConversationType==='C2C'">
        正在呼叫{{toAccount}}...
      </div>
      <div class="title"
           v-if="dialling && currentConversationType==='GROUP'">
        正在呼叫...
      </div>
      ...
      <div v-show="callType === TRTCCalling.CALL_TYPE.AUDIO_CALL && calling"
           class="audio-box">
        <div v-show="calling"
             class="aduio-call">
          <img :src="myAvatar"
          ...
          <p style="text-align: center">
            ...
            <i v-if="isMicOn"
               ...
               style="color: #006FFF;"></i>
            <i v-else
               class="el-icon-turn-off-microphone micr-icon"></i>
          </p>
        </div>
        <div v-for="item in invitedUserInfo "
             class="aduio-call"
             :key="`video-${item.userID}`">
      ...
      <div class="buttons"
           v-if="callType === TRTCCalling.CALL_TYPE.VIDEO_CALL">
        <div :class="isCamOn ? 'videoOn' : 'videoOff'"
             @click="videoHandler"></div>
        <div class="refuse"
             @click="handleDebounce(leave,500)"></div>
        <div :class="isMicOn ? 'micOn' : 'micOff'"
             @click="micHandler"></div>
      </div>
      <div class="buttons"
           v-else>
        ...
        case this.callingTips.callEnd:
          if (this.currentConversationType === this.TIM.TYPES.CONV_GROUP) {
            ...
            call_text =
              callEnd === 0
                ? '取消通话'
                : `结束通话，通话时长：${formatDuration(
                    callEnd || this.duration
                  )}`
            break
          }
